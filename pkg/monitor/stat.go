...
// GetProcesses は引数のinodeの数値をもとにプロセスを検索し，ProcessWithPath型のスライスでで情報を返却します．
func GetProcesses(inode uint32) (*[]ProcessWithPath, error) {
...
func ExistInode(fdPath *string, inode uint32) bool {
	...
	for _, file := range files {
		...
		if err != nil {
			fmt.Println(err)
			continue
		} else if i := strconv.FormatUint(uint64(inode), 10); strings.Contains(str, i) {
			return true
		}
	}
	return false
